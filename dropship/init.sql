...
INSERT INTO users (user_id, name, email, password, created_at, cpf, phone, status, birth_date, email_verified_at)
VALUES
...
CREATE TABLE suppliers (
    ...
    tier ENUM ('OFFICIAL', 'ESPECIALIZED', 'VERIFIED', 'NORMAL') NOT NULL,
...
INSERT INTO products (product_id, product_name, description, price, stock, status, img_url, discount, supplier_id)
VALUES
    (1, 'Notebook Gamer', 'Notebook de alto desempenho para jogos', 4500.00, 10, 'ACTIVE', 'https://m.media-amazon.com/images/I/51Wv-tEUn6L._AC_SX679_.jpg', 10.0, 1),
    (2, 'Smartphone X', 'Smartphone moderno com câmera avançada e alto desempenho', 2200.00, 20, 'ACTIVE', 'https://d3qoj2c6mu9s8x.cloudfront.net/Custom/Content/Products/40/06/4006975_smartphone-apple-iphone-x-5-8-camera-12mp-dual-frontal-7mp-com-ios-11-prata-256gb_m2_637223855454369999.webp', 12.0, 1),
    (3, 'Fone de Ouvido Bluetooth', 'Fone de ouvido sem fio com bateria de longa duração', 150.00, 50, 'ACTIVE', 'https://m.media-amazon.com/images/I/51olNZRjn+L._AC_SY300_SX300_.jpg', 25.0, 1);
...
CREATE TABLE supplier_user (
    ...
    CONSTRAINT fk_su_supplier FOREIGN KEY (supplier_id) REFERENCES suppliers (supplier_id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_su_user FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE ON UPDATE CASCADE,
    UNIQUE KEY uq_supplier_user (supplier_id, user_id)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci;

INSERT INTO supplier_user (supplier_id, user_id, association_date)
VALUES
    (1, 2, NOW()),
    (1, 3, NOW());
...
CREATE TABLE audit_log (
    ...
    entity_name VARCHAR(150) NOT NULL COMMENT 'Nome da entidade auditada',
    entity_id BIGINT NULL COMMENT 'ID da entidade afetada',
    action_type VARCHAR(50) NOT NULL COMMENT 'Tipo da ação: CREATE, UPDATE, DELETE',
    saved_by VARCHAR(200) NULL COMMENT 'Usuário/email obtido do token JWT',
    snapshot_json LONGTEXT NULL COMMENT 'JSON contendo o estado ou diferenças da entidade',
    timestamp DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Momento da ação',
